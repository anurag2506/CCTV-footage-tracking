...
def process_video(video_path):
    output_folder = f"frames_no_coat{os.path.splitext(video_path)[0]}"
    ...
    return zip_path, saved_frame_count

st.title("Coat Detection System")
...
if uploaded_file is not None:
    ...
    with st.spinner("Processing video..."):
        zip_path, saved_frame_count = process_video(video_path)
    
    st.success("Video processing completed!")
    st.write(f"Frames without coats detected: {saved_frame_count}")
    
    with open(zip_path, "rb") as file:
        st.download_button(label="Download ZIP", data=file, file_name=os.path.basename(zip_path), mime="application/zip")
